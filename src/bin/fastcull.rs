...
fn run() -> Result<(), String> {
    ...
    while let Some(arg) = args.next() {
        match arg.to_str() {
            Some("--help" | "-h") => {
                println!("ARW Sprint for Intel Mac\n\nfastcull [folder | photo.ARW] [--cache-mb 256|512|1024|2048]\n\nCmd+O opens a folder. Arrow keys browse; Shift+Space goes back.\nSpace/X moves the displayed photo and XMP into _Rejected, then advances. 1–5 rate.\nF fullscreen, Z fit/100%, S/+ zoom in, D/- zoom out, wheel/pinch zoom, drag pan.\nL toggles keeping zoom and position (on by default), hold P peeks at 100%, C pins/closes comparison, Shift+C replaces reference.\nA toggles auto-advance after ratings; Cmd+Z undoes a rating or local move this session.\n] brightens, [ darkens by 1/3 stop; backslash resets viewing brightness.\nLast photo, brightness and L/A preferences resume on launch.\nFASTCULL_LOG=1 prints pipeline timings and cache statistics.\n\n--smoke-test NEW_OUTPUT_FOLDER captures only ARW Sprint's rendered frames,\nexercises read-only navigation/zoom/resize, then exits. Never changes ratings or moves files.");
                return Ok(());
            }
            Some("--cache-mb") => {
